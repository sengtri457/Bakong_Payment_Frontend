    ...
    <div class="store-wrapper">
      ...
      <main class="landing-luxe fade-in">
        <header class="mission-luxe">
          <span class="mission-tag">THE SCENT</span>
          <h2>Signature <span>Fragrance.</span></h2>
          <p>Olfactory journeys crafted to evoke memory, emotion, and unparalleled presence.</p>
        </header>
          ...
          <div class="luxe-grid" *ngIf="!loading()">
            <div class="luxe-card" *ngFor="let p of filteredProducts(); let i = index" (click)="viewProduct(p)">
              <div class="luxe-img-box">
                <div class="luxe-badge organic" *ngIf="i % 2 === 0">Natural</div>
              ...
              <div class="luxe-card-info">
                ...
                <span class="category">{{ p.sub_category || 'Signature Scent' }}</span>
              </div>
            </div>
          </div>
        </section>
...
export class FragranceComponent implements OnInit {
  ...
  subCategories = computed(() => {
    const list = this.products().filter(p => p.category?.category_name.toLowerCase().includes('fragrance') || p.category?.category_name.toLowerCase().includes('scent'));
    const subs = Array.from(new Set(list.map(p => p.sub_category).filter(Boolean))) as string[];
    return subs.sort();
  });

  filteredProducts = computed(() => {
    let list = this.products().filter(p => p.category?.category_name.toLowerCase().includes('fragrance') || p.category?.category_name.toLowerCase().includes('scent'));
