    ...
    <div class="store-wrapper">
      ...
      <main class="landing-luxe fade-in">
        <header class="mission-luxe">
          <span class="mission-tag">THE RITUAL</span>
          <h2>Skincare <span>Essentials.</span></h2>
          <p>Potent formulas designed to clarify, hydrate, and restore your skin's natural vitality.</p>
        </header>
          ...
          <div class="luxe-grid" *ngIf="!loading()">
            <div class="luxe-card" *ngFor="let p of filteredProducts(); let i = index" (click)="viewProduct(p)">
              <div class="luxe-img-box">
                <div class="luxe-badge" *ngIf="i % 4 === 0">New</div>
              ...
              <div class="luxe-card-info">
                ...
                <span class="category">{{ p.sub_category || 'Skin Ritual' }}</span>
              </div>
            </div>
          </div>
        </section>
...
export class SkincareComponent implements OnInit {
  ...
  subCategories = computed(() => {
    const list = this.products().filter(p => 
      p.category?.category_name.toLowerCase().includes('skincare') || 
  ...
  filteredProducts = computed(() => {
    ...
    if (search) {
      list = list.filter(p => p.product_name.toLowerCase().includes(search) || p.description?.toLowerCase().includes(search));
    }
